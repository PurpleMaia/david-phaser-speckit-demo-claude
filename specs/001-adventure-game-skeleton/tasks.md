...
- [ ] T002 Install Phaser 3 (^3.80.0) and Vite (^5.0.0) as dependencies
- [ ] T003 [P] Create directory structure (src/, src/scenes/, src/svg/, src/utils/, public/)
- [ ] T004 [P] Create public/index.html with game-container div and script tag for src/main.js
- [ ] T005 [P] Create vite.config.js with basic configuration
- [ ] T006 [P] Create .gitignore with node_modules, dist, .DS_Store
- [ ] T007 [P] Add npm scripts to package.json (dev: vite, build: vite build, preview: vite preview)
...
- [ ] T008 Create svgToDataUri utility function in src/utils/svgLoader.js
- [ ] T009 [P] Create createPlayerSvg function in src/svg/player.js (blue circle with triangle indicator)
- [ ] T010 [P] Create createNpcSvg function in src/svg/npc.js (purple square)
- [ ] T011 [P] Create createChestSvg function in src/svg/chest.js (yellow/gold chest icon)
- [ ] T012 [P] Create createWallSvg function in src/svg/wall.js (gray wall tile)
- [ ] T013 Create src/main.js with Phaser game configuration (800x600, Arcade physics, gravity 0)
- [ ] T014 Create MainScene class skeleton in src/scenes/MainScene.js (constructor, init, preload, create, update methods)
- [ ] T015 Import and register MainScene in src/main.js Phaser config
...
- [ ] T016 [P] [US1] Implement SVG asset loading in MainScene.preload() using svgToDataUri for player and wall sprites
- [ ] T017 [US1] Create physics world bounds (800x600) in MainScene.create()
- [ ] T018 [US1] Create player sprite with physics at position (400, 300) in MainScene.create()
- [ ] T019 [US1] Enable player collision with world bounds in MainScene.create()
- [ ] T020 [US1] Create static wall group and add 4 wall rectangles (top, right, bottom, left) using wall texture in MainScene.create()
- [ ] T021 [US1] Add collision between player and walls in MainScene.create()
- [ ] T022 [US1] Set up keyboard input (cursors for arrow keys, WASD keys) in MainScene.create()
- [ ] T023 [US1] Implement movement logic in MainScene.update() responding to arrow keys (160 px/s velocity)
- [ ] T024 [US1] Implement movement logic in MainScene.update() responding to WASD keys (160 px/s velocity)
- [ ] T025 [US1] Test player movement and collision - verify FR-001 through FR-006
...
- [ ] T026 [P] [US2] Load NPC SVG texture in MainScene.preload() using svgToDataUri
- [ ] T027 [US2] Create NPC sprite at position (e.g., 500, 200) in MainScene.create() with interaction radius and dialog text properties
- [ ] T028 [US2] Create checkProximity helper method in MainScene (checks distance between player and target < interaction radius)
- [ ] T029 [US2] Create createDialogBox helper method in MainScene returning Phaser Container with background rectangle and text object
- [ ] T030 [US2] Initialize dialog box (hidden) in MainScene.create()
- [ ] T031 [US2] Set up interaction key inputs (E and Space) in MainScene.create()
- [ ] T032 [US2] Initialize gameState object in MainScene.init() with dialogOpen=false and currentInteractable=null properties
- [ ] T033 [US2] Add proximity detection for NPC in MainScene.update() using checkProximity method
- [ ] T034 [US2] Create showDialog helper method in MainScene (sets text, makes visible, updates gameState, stops player)
- [ ] T035 [US2] Create hideDialog helper method in MainScene (hides dialog, updates gameState)
- [ ] T036 [US2] Create handleInteraction method in MainScene (checks dialog state, shows/hides dialog for NPC)
- [ ] T037 [US2] Add interaction key handler in MainScene.update() calling handleInteraction when E or Space pressed
- [ ] T038 [US2] Disable player movement when dialog is open in MainScene.update()
- [ ] T039 [US2] Test NPC interaction - verify FR-007, FR-009, FR-010, FR-011, FR-012
...
- [ ] T042 [US3] Add proximity detection for chest in MainScene.update() using checkProximity method (only if not collected)
...
- [ ] T045 [US3] Update currentInteractable logic in MainScene.update() to prioritize NPC over chest when both are nearby
- [ ] T046 [US3] Ensure chest cannot be collected twice (check isCollected flag before interaction)
- [ ] T047 [US3] Test object collection - verify FR-008, FR-013, FR-014, FR-015
...
- [ ] T048 [P] Update repository README.md with project description, prerequisites, setup instructions (npm install, npm run dev)
- [ ] T049 [P] Add background color or floor texture to game canvas in src/main.js config
- [ ] T050 Test all three user stories end-to-end (movement → NPC interaction → chest collection)
- [ ] T051 Verify success criteria SC-001: Player can move across room in < 5 seconds
- [ ] T052 Verify success criteria SC-002: Complete all interactions in < 60 seconds
- [ ] T053 Verify success criteria SC-003: Game loads in < 3 seconds (use browser DevTools)
- [ ] T054 Verify success criteria SC-004: Interaction feedback < 0.5 seconds
- [ ] T055 Verify success criteria SC-005: Count source code lines excluding dependencies (should be < 500 lines)
- [ ] T056 Verify constitutional compliance: All assets are code-generated SVGs (no bitmap files in repository)
- [ ] T057 Verify constitutional compliance: Only 2 dependencies (Phaser + Vite) in package.json
- [ ] T058 Remove any console.log statements or debug code from src/ files
- [ ] T059 Test edge cases: rapid key presses, boundary interactions, simultaneous interactions
- [ ] T060 Run production build (npm run build) and verify dist/ output works
